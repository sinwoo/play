...
var menu = (function () 
{
    ...
    var menuInit = function()
    {
		...
		{
			//대메뉴 번호 추출
			if(subArr.indexOf(i+1) !== -1) count ++;
			//소메뉴 번호 추출
            if(listArr[count-1].indexOf(i+1) !== -1) listId = listArr[count-1].indexOf(i+1);
        ...
        $('.tMenuBtn').each(function(idx)
        {
            ...
            if(p_id !== pageInfoArray[NOWPAGE][0])
            {
        ...
        $('.sMenuBtn').each(function(idx)
        {
            ...
            var curId = pageInfoArray[NOWPAGE][0] + '_' + pageInfoArray[NOWPAGE][1];
