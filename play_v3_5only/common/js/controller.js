...
var controller = (function () {
    ...
    var setController = function () {
        ...
        //이전 버튼s - VIDEO_LIST 기반으로 대체되어 비활성화
        /*
        ...
        */
        ...
        //다음 버튼 - VIDEO_LIST 기반으로 대체되어 비활성화
        /*
        ...
        */
        ...
        //다음말풍선 알림 클릭시에도 다음버튼 클릭 이벤트 실행 - VIDEO_LIST 기반으로 수정
        ...
        function seekTimeBookMk() {
            ...
            // bookMrkArr가 없거나 CHAPTER가 없으면 함수 종료
            if (!bookMrkArr || !bookMrkArr[CHAPTER]) {
                return;
            }

            const element_0 = bookMrkArr[CHAPTER][0] ? bookMrkArr[CHAPTER][0][1] : 0;
            const element_1 = bookMrkArr[CHAPTER][1] ? bookMrkArr[CHAPTER][1][1] : 0;
            const element_2 = bookMrkArr[CHAPTER][2] ? bookMrkArr[CHAPTER][2][1] : 0;
        ...
        function moveProgressBar(point) {
            console.log('moveProgressBar 호출됨, point:', point);
            
            // media가 존재하고 duration이 유효한지 확인
            if (!media || !media[0] || !media[0].duration || isNaN(media[0].duration)) {
                console.log('media 또는 duration이 유효하지 않음');
                return;
            }
            ...
            console.log('계산된 퍼센트:', per + '%');
            console.log('현재 duration:', media[0].duration);

            $('.progress-range').css({ width: per + '%' });
            media[0].currentTime = (media[0].duration * per) / 100;

            console.log('설정된 currentTime:', media[0].currentTime);

            // 재생 상태 유지
            if (state === 'play') {
                media[0].play().then(function() {
                    console.log('타임라인 이동 후 재생 성공');
                }).catch(function(error) {
                    console.log('타임라인 이동 후 재생 실패:', error);
                });
            }
        }
